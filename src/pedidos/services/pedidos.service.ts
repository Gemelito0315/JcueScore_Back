...
export class PedidosService {
    ...
    async findAll(usuarioId: number, role: string) {
    ...
    async findOne(id: number) {
        return this.pedidoRepository.findOne({
            where: { id },
            relations: ['usuario', 'items', 'items.product', 'recurso', 'venue', 'garitero']
        });
    }

    async create(usuarioId: number, createPedidoDto: any) {
        const { items, recursoId, notas, metodoPago, direccionEntrega } = createPedidoDto;
        ...
        const pedido = this.pedidoRepository.create({
            usuarioId,
            venueId: usuario.club?.id, // Asociar al club del usuario
            recursoId,
    ...
    async updateStatus(pedidoId: number, estado: string, gariteroId?: number) {
        ...
        if (!validTransitions[pedido.estado].includes(estado)) {
            throw new BadRequestException(`Transición inválida de ${pedido.estado} a ${estado}`);
        }
        ...
        if (estado === EstadoPedido.EN_PREPARACION) {
            ...
            if (pedido.fechaPreparacion) {
                const diff = pedido.fechaEntregado.getTime() - pedido.fechaPreparacion.getTime();
    ...
    async markAsDelivered(pedidoId: number, body?: { metodoPago?: string; pagado?: number }) {
    ...
    async getDailyStats(venueId: number) {
        ...
        const stats = {
            ...
            pedidosPendientes: pedidosHoy.filter(p => p.estado === EstadoPedido.PENDIENTE).length,
            pedidosEnPreparacion: pedidosHoy.filter(p => p.estado === EstadoPedido.EN_PREPARACION).length,
            pedidosListos: pedidosHoy.filter(p => p.estado === EstadoPedido.LISTO).length,
            pedidosEntregados: pedidosHoy.filter(p => p.estado === EstadoPedido.ENTREGADO).length,
            ingresosTotales: pedidosHoy.reduce((sum, p) => sum + p.total, 0),
            tiempoPromedioPreparacion: 0,
            productosVendidos: 0
        };

        // Calcular tiempo promedio de preparación
        const pedidosConTiempo = pedidosHoy.filter(p => p.tiempoPreparacionMinutos > 0);
        if (pedidosConTiempo.length > 0) {
            stats.tiempoPromedioPreparacion = Math.round(
                pedidosConTiempo.reduce((sum, p) => sum + p.tiempoPreparacionMinutos, 0) / pedidosConTiempo.length
            );
        }

        // Calcular productos vendidos
        stats.productosVendidos = pedidosHoy.reduce((sum, p) => 
            sum + (p.items?.reduce((sumItems, item) => sumItems + item.cantidad, 0) || 0), 0
        );
    ...
    async getTopProducts(venueId: number, limit: number = 10) {
        ...
        const query = this.pedidoItemRepository.createQueryBuilder('item')
