...
import {
  ...
  TipoTransaccion,
  FuenteTransaccion,
} from '../entities/transaccion-lealtad.entity';
...
export class LealtadService {
  ...
  async minarCoins(
    ...
    if (ultimaTransaccion) {
      const tiempoDesdeUltima =
        Date.now() - ultimaTransaccion.createdAt.getTime();
      if (tiempoDesdeUltima < 55000) {
        // 55 segundos mínimo
  ...
  private getProximoNivel(coins: number): {
    nombre: string;
    coinsRequeridos: number;
    coinsActuales: number;
  } {
    if (coins >= 10000)
      return { nombre: 'Máximo', coinsRequeridos: 0, coinsActuales: coins };
    if (coins >= 5000)
      return {
        nombre: 'Diamante',
        coinsRequeridos: 10000,
        coinsActuales: coins,
      };
    if (coins >= 2000)
      return { nombre: 'Oro', coinsRequeridos: 5000, coinsActuales: coins };
    if (coins >= 500)
      return { nombre: 'Plata', coinsRequeridos: 2000, coinsActuales: coins };
