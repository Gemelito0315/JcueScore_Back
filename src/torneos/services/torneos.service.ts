...
import { InscripcionTorneo, EstadoInscripcion } from '../entities/inscripcion-torneo.entity';
import { PartidoTorneo, EstadoPartido, FaseTorneo } from '../entities/partido-torneo.entity';
...
export class TorneosService {
    ...
    async findOne(id: number) {
        return this.torneoRepository.findOne({
            where: { id },
            relations: ['inscripciones', 'inscripciones.jugador', 'partidos', 'partidos.jugador1', 'partidos.jugador2']
        });
    }
    ...
    async inscribirJugador(torneoId: number, body: { jugadorId: number; handicap?: number }) {
    ...
    private async generarRoundRobin(torneoId: number, inscripciones: InscripcionTorneo[]) {
    ...
    private async generarEliminacionDirecta(torneoId: number, inscripciones: InscripcionTorneo[]) {
        const partidos: PartidoTorneo[] = [];
    ...
    private async actualizarEstadisticasJugadores(partido: PartidoTorneo) {
        ...
        jugador2.eloRating = Math.round(jugador2.eloRating + kFactor * ((1 - actualScore1) - (1 - expectedScore1)));
